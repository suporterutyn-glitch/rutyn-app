    ...
    <div className="app-shell app-bg-pro flex flex-col">
      <div className="relative z-10 flex flex-col min-h-dvh px-6 pt-[calc(env(safe-area-inset-top)+16px)]">
        ...
        <div className="flex-1 flex flex-col justify-center gap-6">
          <h1 className="text-white text-rt-22 font-bold">Recuperar senha</h1>
          {sent ? (
            <p className="text-white/80 text-rt-14">Se o e-mail existir, enviamos um link para redefinir sua senha.</p>
          ) : (
            <form onSubmit={submit} className="flex flex-col gap-4">
              ...
              <button className="btn-save mt-4" disabled={loading}>
                {loading ? t('loading') : 'Enviar e-mail'}
              </button>
            </form>
          )}
        </div>
